 ...
 * <ul>
 * <li>When the main window's close button is clicked, it ensures all tasks are complete. If some are still unchecked,
 * it will ask for confirmation before closing the application.</li>
...
public class ApplicationClosingListener extends WindowAdapter {
     ...
     * {@inheritDoc}
     * <p/>
     * When the window is closed, this implementation asks for confirmation if some tasks are not complete. If all tasks
     * are complete, it just closes the application.
     * <p/>
    ...
    public void windowClosing(WindowEvent e) {
        if (TaskManager.getInstance().areAllTasksDone()) {
            ...
            /*
             * For window closing, if some tasks are not yet completed, ask for confirmation.
             */
            int exit = JOptionPane.showConfirmDialog(e.getComponent(),
                    MessageBundle.get("confirm.skip.unfinished.body"),
                    MessageBundle.get("confirm.skip.unfinished.title"), JOptionPane.YES_NO_OPTION);
